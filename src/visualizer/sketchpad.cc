...
Sketchpad::Sketchpad(const vec2& top_left_corner, size_t num_pixels_per_side,
    ...
    : top_left_corner_(top_left_corner),
      ...
      image_(num_pixels_per_side, num_pixels_per_side){}
...
void Sketchpad::Draw() const {
  for (size_t row = 0; row < num_pixels_per_side_; ++row) {
    for (size_t col = 0; col < num_pixels_per_side_; ++col) {
      ...
      // check if the pixel at (row, col) is currently shaded
      if (image_.IsPixelShaded(row, col)) {
  ...
  for (size_t row = 0; row < num_pixels_per_side_; ++row) {
    for (size_t col = 0; col < num_pixels_per_side_; ++col) {
      ...
      if (glm::distance(brush_sketchpad_coords, pixel_center) <=
          brush_radius_) {
        // shade in the pixel at (row, col)
        image_.SetPixelColor(row, col, '#');
      }
    }
  }
}
...
Image Sketchpad::GetImage() const {
  return image_;
}
